...
<div class="mt-6 space-y-auto">
    <div class="grid grid-cols-1 border-b border-gray-200 py-5 sm:flex items-start">
        ...
        <div class="flex justify-between w-full mt-4 sm:col-span-2 items-start space-x-5">
            <div>
                @if($lab->profile_photo)
                <div>
                    <img src="{{ Storage::url($lab->profile_photo) }}" class="h-12 w-12 rounded-full" alt="">
                </div>
                @else
                <div>
            ...
            <div class="inline-flex items-center space-x-3">
                <form action="{{ route('lab.profile.update', Auth()->id()) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
    ...
    <div class="grid grid-cols-1 border-b border-gray-200 py-5 sm:flex items-start">
        ...
        <div class="w-full mt-4 sm:col-span-2 items-start space-x-5">
            <div>
                <input type="text" class="block w-full p-2 border-gray-300 rounded-lg bg-gray-50 sm:text-md focus:ring-green-500 focus:border-green-500" placeholder="{{ $lab->name }}">
            </div>
        </div>
    </div>
    <div class="grid grid-cols-1 border-b border-gray-200 py-5 sm:flex items-start">
...
<div class="mt-4 space-y-1">
    <h1 class="font-bold text-xl">Praktikum</h1>
