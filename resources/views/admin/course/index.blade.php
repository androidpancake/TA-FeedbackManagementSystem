        ...
        <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $data->id }}</th>
                            <td class="px-4 py-3">{{ $data->code }}</td>
                            <td class="px-4 py-3">{{ $data->name }}</td>
                            

                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="dropdownID{{ $data->id }}" data-dropdown-toggle="dropdownAction{{ $data->id }}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none" type="button">
                                ...
                                <div id="dropdownAction{{ $data->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="dropdownID{{ $data->id }}">
                                        <li>
                                            <a href="{{ route('admin.course.detail', $data->id) }}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 text-gray-700">
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Showing
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $course->firstItem() }}-{{ $course->lastItem() }}</span>
                    of
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $course->total() }}</span>
                </span>
                <ul class="inline-flex items-stretch -space-x-px">
                    <!-- previous -->
                    @if ($course->onFirstPage())
                        <li class="opacity-50 pointer-events-none">
                            <span class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700" aria-disabled="true">
                    ...
                    @else
                        <li>
                            <a href="{{ $course->previousPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">
                                <span class="sr-only">Previous</span>
                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                </svg>
                    ...
                    <!-- page -->
                    @foreach ($course as $page)
                        <li>
                            <a href="{{ $page->url }}" class="flex items-center justify-center text-sm py-2 px-3 leading-tight {{ $page->isActive ? 'font-semibold text-gray-900' : 'text-gray-500' }} bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700">{{ $page->number }}</a>
                        </li>
                    @endforeach
                    ...
                    @if ($course->hasMorePages())
                        <li>
                            <a href="{{ $course->nextPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">
                    ...
                    @else
                        <li class="opacity-50 pointer-events-none">
                            <span class="flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700" aria-disabled="true">
                                <span class="sr-only">Next</span>
                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
