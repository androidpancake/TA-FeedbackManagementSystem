...
<div class="grid justify-center pb-8">
    ...
    <div class="mt-4 pb-10">
        ...
        <form action="{{ route('mahasiswa.complaint.store') }}" method="POST" class="space-y-8" enctype="multipart/form-data">
            ...
            <div>
                ...
                <select name="category_id" class="w-full bg-white border {{ $errors->has('category_id') ? 'border-red-400' : 'border-gray-300' }} hover:bg-gray-100 focus:border-green-500 focus:ring-green-500 rounded-lg text-sm p-3 text-start inline-flex justify-between items-center">
            ...
            <div>
                ...
                <input type="text" name="subject" id="large-input" class="block w-full p-3 text-gray-900 border {{ $errors->has('category_id') ? 'border-red-400' : 'border-gray-300' }} rounded-lg bg-white sm:text-md focus:ring-green-500 focus:border-green-500 hover:bg-gray-50 text-sm" placeholder="Tulis subjek">
                @error('subject')
                <div class="pt-2 text-sm text-red-600">
                    {{ $message }}
                </div>
            ...
            <div>
                ...
                <textarea id="message" name="content" rows="4" class="block p-3 w-full text-sm text-gray-900 bg-white rounded-lg border {{ $errors->has('category_id') ? 'border-red-400' : 'border-gray-300' }} hover:bg-gray-50 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Tulis komentar anda..."></textarea>
                @error('content')
                <div class="pt-2 text-sm text-red-600">
                    {{ $message }}
                </div>
            ...
            <div>
                ...
                <input name="file" class="bg-gray-300 block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="multiple_files" type="file" required>
                @error('file')
                <div class="pt-2 text-sm text-red-600">
                    {{ $message }}
                </div>
    ...
    <div class="flex justify-end">
        <button data-modal-target="konfirmasi" data-modal-toggle="konfirmasi" class="bg-green-500 hover:bg-green-800 py-2 px-4 rounded text-white" type="button">Kirim Pengaduan</button>
    </div>
