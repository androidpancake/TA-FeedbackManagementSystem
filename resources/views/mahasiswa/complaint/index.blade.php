                    ...
                    <div class="flex sm:flex justify-between items-center space-x-2">
                        ...
                        <div class="inline-flex space-x-2">
                            ...
                            <div id="dropdownWait{{ $data->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                                <ul aria-labelledby="dropdownDelete" class="py-2 text-sm text-gray-700">
                                    <li>
                                        <button onclick="event.preventDefault()" data-modal-target="modalWait{{ $data->id }}" data-modal-toggle="modalWait{{ $data->id }}" class="w-full px-4 py-2 text-base hover:bg-gray-100">Hapus komplain</button>
                                    </li>
                                </ul>
                            </div>
                            <!-- modal -->
                                        ...
                                        <div class="p-6">
                                            ...
                                            <div class="flex flex-row space-x-3 w-full">
                                                <form class="flex flex-row space-x-3 w-full" method="POST" action="{{ route('mahasiswa.complaint.delete' , $data->id) }}">
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                    <button data-modal-hide="modalWait{{ $data->id }}" type="submit" class="w-full text-center justify-center text-white bg-red-500 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-blue-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 mr-2">
                                                        Hapus Keluhan
                                                    </button>
                                                </form>
                                            </div>
                    ...
                    <div class="flex sm:flex justify-between items-center space-x-2">
                        ...
                        <div class="inline-flex space-x-2">
                            ...
                            <div id="dropdownRead{{ $data->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                                <ul aria-labelledby="dropdownDelete" class="py-2 text-sm text-gray-700">
                                    <li>
                                        <button onclick="event.preventDefault()" data-modal-target="modalRead{{ $data->id }}" data-modal-toggle="modalRead{{ $data->id }}" class="w-full px-4 py-2 text-base hover:bg-gray-100">Hapus komplain</button>
                                    </li>
                                </ul>
                            </div>
                            <!-- modal -->
                            <div id="modalRead{{ $data->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="z-20 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button onclick="event.preventDefault()" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="modalRead{{ $data->id }}">
                                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                            </svg>
                                            <span class="sr-only">Close modal</span>
                                        ...
                                        <div class="p-6">
                                            ...
                                            <div class="flex flex-row space-x-3 w-full">
                                                <form class="flex flex-row space-x-3 w-full" method="POST" action="{{ route('mahasiswa.complaint.delete' , $data->id) }}">
                                                    <button data-modal-hide="modalRead{{ $data->id }}" type="button" class="w-full text-center justify-center text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                                        Batal
                                                    </button>
                                                    @csrf
                                                    @method('DELETE')
                                                    <button data-modal-hide="modalRead{{ $data->id }}" type="submit" class="w-full text-center justify-center text-white bg-red-500 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-blue-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 mr-2">
                                                        Hapus Keluhan
                                                    </button>
                                                </form>
                                            </div>
        ...
        <div class="divide-y divide-gray-200">
            ...
            <a href="{{ route('mahasiswa.complaint.detail', $data->id) }}" class="block">
                <div class="sm:bg-white py-6 px-8 hover:bg-gray-50">
                    <div class="flex sm:flex justify-between items-center space-x-2">
                        <div class="flex md:flex-row items-center justify-start space-x-2">
                            <p class="text-sm font-medium text-gray-600">Prodi</p>
                        </div>
                        <div class="inline-flex space-x-2">
                            <p class="text-sm font-medium text-gray-400">{{ date('D, d M Y, H:i', strtotime($data->created_at)) }}</p>
                            <button onclick="event.preventDefault()" id="dropdownDelete" data-dropdown-toggle="dropdownProcess{{ $data->id }}" class="hover:bg-gray-200 rounded-full focus:ring-2 ring-green-600 ring-inset z-10">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="#000000" viewBox="0 0 256 256">
                                    <path d="M140,128a12,12,0,1,1-12-12A12,12,0,0,1,140,128Zm56-12a12,12,0,1,0,12,12A12,12,0,0,0,196,116ZM60,116a12,12,0,1,0,12,12A12,12,0,0,0,60,116Z"></path>
                                </svg>
                            </button>
                            <div id="dropdownProcess{{ $data->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                                <ul aria-labelledby="dropdownDelete" class="py-2 text-sm text-gray-700">
                                    <li>
                                        <button onclick="event.preventDefault()" data-modal-target="modalProcess{{ $data->id }}" data-modal-toggle="modalProcess{{ $data->id }}" class="w-full px-4 py-2 text-base hover:bg-gray-100">Hapus komplain</button>
                                    </li>
                                </ul>
                            </div>
                            <!-- modal -->
                            <div id="modalProcess{{ $data->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="z-20 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button onclick="event.preventDefault()" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="modalProcess{{ $data->id }}">
                                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                            </svg>
                                            <span class="sr-only">Close modal</span>
                                        ...
                                        <div class="p-6">
                                            ...
                                            <div class="flex flex-row space-x-3 w-full">
                                                <form class="flex flex-row space-x-3 w-full" method="POST" action="{{ route('mahasiswa.complaint.delete' , $data->id) }}">
                                                    <button data-modal-hide="modalProcess{{ $data->id }}" type="button" class="w-full text-center justify-center text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                                        Batal
                                                    </button>
                                                    @csrf
                                                    @method('DELETE')
                                                    <button data-modal-hide="modalProcess{{ $data->id }}" type="submit" class="w-full text-center justify-center text-white bg-red-500 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-blue-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 mr-2">
                                                        Hapus Keluhan
                                                    </button>
                                                </form>
                                            </div>
                    ...
                    <div class="flex sm:flex justify-between items-center space-x-2">
                        ...
                        <div class="inline-flex space-x-2">
                            ...
                            <div id="dropdownDone{{ $data->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                                <ul aria-labelledby="dropdownDelete" class="py-2 text-sm text-gray-700">
                                    <li>
                                        <button onclick="event.preventDefault()" data-modal-target="modalDone{{ $data->id }}" data-modal-toggle="modalDone{{ $data->id }}" class="w-full px-4 py-2 text-base hover:bg-gray-100">Hapus komplain</button>
                                    </li>
                                </ul>
                            </div>
                            <!-- modal -->
